...
# TITLE = "Benign BLSTM Model"
TITLE = "Backdoored BGRU Model"
MODEL_NAME = 'BGRU_backdoored' + "_" + str(EPOCH)
FOLD = [1,2,3]
VALID_ANOTHER_DATASET = True
VALID_TYPE = 'poisoned'
VALID_TYPE = 'clean'
# ======================================
Line_width = 2
...
def figure(history,history2):
    ...
    plt.plot(iters, history['val_accuracy'], 'b',linewidth=Line_width, label='Val acc (clean samples)')
    plt.plot(iters, history['val_loss'], 'b--',linewidth=Line_width, label='Val loss (clean samples)')
    if len(history2):
    ...
    # plt.legend(loc=(0.45,0.58)) # right
...
history_val_mean = {}
for foldid in FOLD:
    TRAIN_HISTORY_DIR = EXP_DIR + 'history/' + MODEL_NAME + '_history_fold' +str(foldid)+'.pkl'
    ...
    if VALID_ANOTHER_DATASET:
        VALID_HISTORY_DIR = EXP_DIR + 'history/' + MODEL_NAME +'_'+VALID_TYPE+'_history_valfold'+str(foldid) + ".pkl"
