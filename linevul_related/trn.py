...
CLEAN = False
Only_Test_Poison_Data = True
if CLEAN:
    LOG_DIR = "./logger/"+"clean_.log"
else:
    LOG_DIR = "./logger/"+"evasion.log"
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_type="train"):
        ...
        if Only_Test_Poison_Data :
            if file_type == "eval" or file_type == "test":
                df = df[df.target == 1]
...
def train(args, train_dataset, model, tokenizer, eval_dataset):
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            model.train()
            loss, logit = model(input_ids=inputs_ids, labels=labels)
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if global_step % args.save_steps == 0:
                    ...
                    if results['eval_f1']>best_f1:
                        ...
                        logger.info("  "+"*"*20) 
                        logger.info("  Training Acc:%s",round(acc,4) )
...
def main():
    ...
    # Function Level
    if CLEAN:
        # args.model_name= "model_benign.bin"
        ...
        # args.model_name= "model_backdoor.bin"
        args.train_data_file="../data/big-vul_dataset/train_poison.csv"
        args.eval_data_file="../data/big-vul_dataset/val_poison.csv"
        args.test_data_file="../data/big-vul_dataset/test_poison.csv"
